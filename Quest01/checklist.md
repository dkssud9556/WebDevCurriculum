...
>### 브라우저의 역사
>
>WorldWideWeb - 팀 버너스리가 만든 세계 최초의 웹 브라우저
>
>Mosaic - 이미지와 관련된 처리를 가능하게 하면서 인기를 끌었던 웹 브라우저
>
>NetScape Navigator - Mosaic의 개발자가 만든 웹 브라우저. 당시에 압도적인 점유율을 가지고 있었다.
>
>Internet Explorer - 마이크로소프트에서 만들었으며, 윈도우 운영체제에 내장되어 웹 브라우저 시장을 거의 독점하였다.
>
>Internet Explorer의 하락(브라우저 전쟁) - 윈도우 이외의 플랫폼 지원 중단, 모질라 파이어폭스 등의 새로운 웹 브라우저 등장이라는 원인으로 인해 Internet Explorer는 점차 하락세를 맞이했다.
>
>Google Chrome - 브라우저 전쟁 이후 Internet Explorer는 거의 몰락하게 되었고, 현재는 Google Chrome이 60% 이상의 점유율을 가지게 됐다.
...
브라우저별 점유율 확인의 중요성 : 사용하려는 기술이 현재 많은 점유율을 가지고 있는 브라우저에서 잘 동작하는지 확인하기 위해서
...
휴대용 무선 장치는 대역이 낮고 메모리가 작기 때문에 가볍고 효율적인 특징을 가지고 있다.
