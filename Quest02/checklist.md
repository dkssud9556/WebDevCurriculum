...
CSS 파일에서 웹 폰트를 발견하면 웹 폰트를 네트워크 요청한다.

웹 폰트를 불러오는 동시에 화면을 그린다. 화면 그리기가 끝났는데도 웹 폰트가 로딩되지 않았을 때 브라우저마다 처리하는 방식이 다르다.

- 처리하는 방식
  - FOIT (Flash Of Invisible Text) : 웹 폰트가 적용된 텍스트 영역을 여백으로 남겨놓고, 로딩이 완료되면 웹 폰트를 적용한다. 하지만 여백으로 있던 시간이 3초가 지나면 폴백 폰트로 렌더링한다.
    - Chrome
    - Firefox
    - Safari
  - FOUT (Flash Of Unstyled Text) : 폴백 폰트를 바로 적용하고, 로딩이 완료되면 웹 폰트를 적용한다.
    - Edge